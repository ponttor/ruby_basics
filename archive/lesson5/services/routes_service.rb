...
module RoutesService
  class << self
    def create_route(available_stations)
      ...
      return puts Translations::ROUTE[:errors][:default] if !route

      puts Translations::ROUTE[:created] % [start_station.name, end_station.name]
      route
    end
    ...
    def apply_action_to_route(route, action, stations)
      case action
      when 1
      ...
      when 2
        ...
        puts Translations::ROUTE.errors.invalid_input
      end
    end

    def select_route(routes)
      Helper.select_from_collection(routes, Translations::ROUTE[:select_route]) do |route|
